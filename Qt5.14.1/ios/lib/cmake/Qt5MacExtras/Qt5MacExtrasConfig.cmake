...
set(Qt5MacExtras_VERSION_STRING 5.14.1)
...
function(_qt5_MacExtras_process_prl_file prl_file_location Configuration lib_deps link_flags)
    set(_lib_deps)
    set(_link_flags)

    set(_qt5_install_libs "${_qt5MacExtras_install_prefix}/lib/")
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5MacExtras_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5MacExtras_${Configuration}_${_lib}_PATH ${_lib})
                    mark_as_advanced(_Qt5MacExtras_${Configuration}_${_lib}_PATH)
                    if(_Qt5MacExtras_${Configuration}_${_lib}_PATH)
                        list(APPEND _lib_deps
                            ${_Qt5MacExtras_${Configuration}_${_lib}_PATH}
                        )
                    else()
...
macro(_populate_MacExtras_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    set(_static_deps
        ${_Qt5MacExtras_STATIC_${Configuration}_LIB_DEPENDENCIES}
    )
    ...
    set(_static_link_flags "${_Qt5MacExtras_STATIC_${Configuration}_LINK_FLAGS}")
    if(_static_link_flags)
        set(_static_link_flags_genex "$<${_genex_condition}:${_static_link_flags}>")
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            set_property(TARGET Qt5::MacExtras APPEND PROPERTY INTERFACE_LINK_OPTIONS
...
if (NOT TARGET Qt5::MacExtras)
    ...
    set(Qt5MacExtras_PRIVATE_INCLUDE_DIRS
        "${_qt5MacExtras_install_prefix}/include/QtMacExtras/5.14.1"
        "${_qt5MacExtras_install_prefix}/include/QtMacExtras/5.14.1/QtMacExtras"
    )
    ...
    foreach(_module_dep ${_Qt5MacExtras_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.14.1 ${_Qt5MacExtras_FIND_VERSION_EXACT}
    ...
    if(NOT Qt5_EXCLUDE_STATIC_DEPENDENCIES)
        _qt5_MacExtras_process_prl_file(
            "${_qt5MacExtras_install_prefix}/lib/libQt5MacExtras_debug.prl" DEBUG
            _Qt5MacExtras_STATIC_DEBUG_LIB_DEPENDENCIES
            _Qt5MacExtras_STATIC_DEBUG_LINK_FLAGS
        )

        _qt5_MacExtras_process_prl_file(
            "${_qt5MacExtras_install_prefix}/lib/libQt5MacExtras.prl" RELEASE
            _Qt5MacExtras_STATIC_RELEASE_LIB_DEPENDENCIES
            _Qt5MacExtras_STATIC_RELEASE_LINK_FLAGS
        )
    endif()
    ...
    _populate_MacExtras_target_properties(RELEASE "libQt5MacExtras.a" "" TRUE)



    _populate_MacExtras_target_properties(DEBUG "libQt5MacExtras_debug.a" "" TRUE)
